...
void E53ST1Init(void)
{
    ...
    IoTGpioSetOutputVal(WIFI_IOT_GPIO_IDX_7, 1);
    usleep(10000);
...
void NMEA_BDS_GPRMC_Analysis(gps_msg *gpsmsg, uint8_t *buf)
{
    ...
    uint32_t temp;
    float rs;
    p4 = (uint8_t *)strstr((const char *)buf, "$GPGGA"); // "$GPRMC",经常有&和GPRMC分开的情况,故只判断GPRMC.
    if (p4 != NULL) {
        ...
        posx = NMEA_Comma_Pos(p4, L80R_CONSTANT_3); // 南纬还是北纬
        if (posx != 0XFF) {
            gpsmsg->nshemi_bd = *(p4 + posx);
        }
        posx = NMEA_Comma_Pos(p4, L80R_CONSTANT_4); // 得到经度
        if (posx != 0XFF) {
            temp = NMEA_Str2num(p4 + posx, &dx);
